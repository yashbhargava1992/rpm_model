...
def radius_compute_and_compare(nu1,nu2,nu3,r_orb_arr,r_per_arr,r_nod_arr,flag_sel,mass_array,spin_array,ms_index,r_guess=10):
	"""
	...
	spin = spin_array[s]
	try:
		r_orb_arr[m][s] = newton_solver(r_guess,nu_phi,nu3[0],mass,spin)
		...
		r_orb_min = np.nan
	try:
		r_per_arr[m][s] = newton_solver(r_guess,nu_per,nu2[0],mass,spin)
		...
		r_per_min = np.nan
	try:
		r_nod_arr[m][s] = newton_solver(r_guess,nu_nod,nu1[0],mass,spin)
